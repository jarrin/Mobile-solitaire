define(['jquery'], function ($) {
	var Engine = function (dev, cb) {
		...
		this.cards.createCardsTo = function(target, faced)
		{
			...
			$(this).each(function(n, card){
				_this.cardsObj.push(new _this.Card(card, $("#" + target), n+1));
			});
			return this;
		}
			...
			switch(gameType)
			{
				case 'klondike':
					...
					var files = [
						"types/"+ gameType +"/game",
						"types/"+ gameType +"/card",
						"text!types/" + gameType + "/game.html"
					];
					require(files, function(Game, Card, html){
						_this.Card = Card;
						$("#game").html(html);
		...
		{
			elements.removeClass("back").trigger("movable");
		}
	...
	Engine.prototype.setScaling = function() {
		...
		$("div.card:not(.sample), .table-stack").css({
